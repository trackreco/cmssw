...
namespace ROOT {
   ...
   */
    ...
    public:
      ...
      inline const T& operator()(unsigned int i, unsigned int j) const { return fArray[i * D2 + j]; }
      inline T& operator()(unsigned int i, unsigned int j) { return fArray[i * D2 + j]; }
   ...
   */
    ...
    namespace rowOffsetsUtils {
      ...
      struct make_indices_impl<I, indices<Indices...>, N> {
        typedef typename make_indices_impl<I + 1, indices<Indices..., I>, N>::type type;
      };
      ...
      constexpr std::array<decltype(std::declval<F>()(std::declval<int>())), sizeof...(I)> do_make(F f, indices<I...>) {
        return std::array<decltype(std::declval<F>()(std::declval<int>())), sizeof...(I)>{{f(I0 + I)...}};
      }
   ...
   */
    ...
    public:
      ...
      inline T& operator()(unsigned int i, unsigned int j) { return fArray[offset(i, j)]; }

      inline /* constexpr */ T const& operator()(unsigned int i, unsigned int j) const { return fArray[offset(i, j)]; }

      inline T& operator[](unsigned int i) { return fArray[off(i)]; }

      inline /* constexpr */ T const& operator[](unsigned int i) const { return fArray[off(i)]; }

      inline /* constexpr */ T apply(unsigned int i) const { return fArray[off(i)]; }
      ...
      inline MatRepSym<T, D>& operator=(const R&) {
        STATIC_CHECK(0 == 1, Cannot_assign_general_to_symmetric_matrix_representation);
        return *this;
      }
...
#pragma omp simd
      ...
      template <class R>
      inline MatRepSym<T, D>& operator+=(const R&) {
        STATIC_CHECK(0 == 1, Cannot_add_general_to_symmetric_matrix_representation);
        return *this;
      }
      inline MatRepSym<T, D>& operator+=(const MatRepSym& rhs) {
        for (unsigned int i = 0; i < kSize; ++i)
      ...
      template <class R>
      inline MatRepSym<T, D>& operator-=(const R&) {
        STATIC_CHECK(0 == 1, Cannot_substract_general_to_symmetric_matrix_representation);
        return *this;
      }
      inline MatRepSym<T, D>& operator-=(const MatRepSym& rhs) {
        for (unsigned int i = 0; i < kSize; ++i)
      ...
      static inline constexpr unsigned int offset(unsigned int i, unsigned int j) {
        //if (j > i) std::swap(i, j);
        return off(i * D + j);
