...
struct RZBox {
  ...
  void print(bool nl = true) {
    printf("r~[%.4f, %.4f] z~[%.4f, %.4f]%s", m_minr, m_maxr, m_minz, m_maxz, nl ? "\n" : "");
  }
...
struct BBS {
  ...
  void print() {
    for (int d = 1; d < Mdet; ++d) {
      ...
      for (int l = 1; l < Xlay[d]; ++l) {
        if (isbrl[d]) {
          printf("  lay %d: cnt=%5d, ", l, cnt[d][l]);
          b[d][l].print();
        } else {
          printf("  pos %d: cnt=%5d, ", l, p[d][l].m_cnt);
          p[d][l].print();
          printf("  neg %d: cnt=%5d, ", l, n[d][l].m_cnt);
          n[d][l].print();
        }
      }
      printf("\n");
  ...
  void load(const char* fname = "bbs.root") {
    ...
    if (!f)
      throw std::runtime_error("tnc::load could not open file");

    BBS* b = 0;
    gFile->GetObject("bbs", b);
    if (!b)
      throw std::runtime_error("tnc::load could not read bbs");
...
void tncdefs() { printf("tncdefs() loaded\n"); }
